...
public class DeployBomb {
    ...
    public static String solution(String x, String y) {
        ...
        while (!a.min(b).equals(new BigInteger("1"))) {
            BigInteger m = a.max(b);
            BigInteger n = a.min(b);
            if (m.mod(n).equals(BigInteger.ZERO)) {
                return "impossible";
            }
            count = count.add(m.divide(n));
            a = m.mod(n);
            b = n;
        }
        return String.valueOf(count.add(a.max(b)).subtract(new BigInteger("1")));
    }
